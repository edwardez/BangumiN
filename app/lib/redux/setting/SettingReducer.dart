...
final settingReducers = combineReducers<SettingState>([
  ...
  TypedReducer<SettingState, UpdateMuteSettingAction>(
      updateMuteSettingReducer),
  TypedReducer<SettingState, MuteUserAction>(
      muteUserReducer),
  TypedReducer<SettingState, UnmuteUserAction>(
      unmuteUserReducer),
  TypedReducer<SettingState, MuteGroupAction>(
      muteGroupReducer),
  TypedReducer<SettingState, UnmuteGroupAction>(
      unmuteGroupReducer),
...
SettingState updateMuteSettingReducer(SettingState settingState,
  ...
  return settingState.rebuild(
          (b) => b..muteSetting.replace(action.muteSetting));
}

SettingState muteUserReducer(SettingState settingState,
    MuteUserAction action) {
  return settingState.rebuild(
          (b) =>
      b
        ..muteSetting.mutedUsers.addAll(
            {action.mutedUser.username: action.mutedUser}));
}
...
SettingState muteGroupReducer(SettingState settingState,
  ...
  return settingState.rebuild(
          (b) =>
      b
        ..muteSetting.mutedGroups.addAll(
            {action.mutedGroup.groupId: action.mutedGroup}));
}

SettingState unmuteGroupReducer(SettingState settingState,
    UnmuteGroupAction action) {
  return settingState.rebuild(
          (b) => b..muteSetting.mutedGroups.remove(action.groupId));
}
...
SettingState importBlockedBangumiUsersConfirmReducer(SettingState settingState,
  ...
  return settingState.rebuild(
          (b) => b..muteSetting.mutedUsers.addAll(users.toMap()));
}
...
SettingState importBlockedBangumiUsersCleanupReducer(SettingState settingState,
  ...
  return settingState.rebuild(
          (b) => b..muteSetting.importedBangumiBlockedUsers = null);
}
