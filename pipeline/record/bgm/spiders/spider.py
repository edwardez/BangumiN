...
class UserSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        nickname = response.xpath(".//*[@class='headerContainer']//*[@class='inner']/a/text()").extract()[0].translate(
            mpa)
        ...
        date = response.xpath(".//*[@id='user_home']/div[@class='user_box clearit']/ul/li[1]/span[2]/text()").extract()[
            0].split(' ')[0]
        date = parsedate(date)
...
class RecordSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if (not response.xpath(".//*[@id='headerProfile']")) or response.xpath(".//div[@class='tipIntro']"):
            return
        uid = int(response.meta['redirect_urls'][0].split('/')[-1]) if 'redirect_urls' in response.meta else int(
            username)
    ...
    def parse_recorder(self, response):
        ...
        for item in items:
            ...
            if item.xpath("./div/p[@class='collectInfo']/span[@class='tip']"):
                item_tags = item.xpath("./div/p[@class='collectInfo']/span[@class='tip']/text()").extract()[0].split(
                    u' ')[2:-1]
            else:
                item_tags = None

            try_match = re.match(r'sstars(\d+) starsinfo',
                                 item.xpath("./div/p[@class='collectInfo']/span[1]/@class").extract()[0])
            if try_match:
            ...
            comment = item.xpath(".//div[@class='text']/text()").extract()[0] if len(
        ...
        if len(items) == 24:
            yield scrapy.Request(getnextpage(response.url), callback=self.parse_recorder,
                                 meta={'uid': response.meta['uid']})
...
class SubjectInfoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        relations = [itm.extract() for itm in
                     response.xpath(".//ul[@class='browserCoverMedium clearit']/li[@class='sep']/span/text()")]
...
class SubjectSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        tplst = [itm.split('/')[-1] for itm in
                 response.xpath(".//*[@id='columnSubjectHomeA']/div[3]/span/a/@href").extract()]
        favcount = [0] * 5
        j = 1
        for i in range(5):
            if not tplst or tplst[0] != statestr[i]:
                ...
                tmpstr = \
                    response.xpath(".//*[@id='columnSubjectHomeA']/div[3]/span/a[" + str(j) + "]/text()").extract()[0]
